...
「打卡」是一个独立栏目，不在上面五个文章的列表里，所以它不会混进首页「最近」、归档和 RSS。页面上会显示：今天该打勾的清单、每个习惯的连续天数、最近完成情况，以及一张最近一年的打卡日历（53 周，横着排，手机上左右滑动）。地址是 **https://bluex888.github.io/checkin/**。

每天记一次：打开后台 → 左侧 **打卡** → 右上角 **+** 新建（日期默认今天，习惯清单已经列好）→ 做完的把「已完成」打开 → 点右上角绿色的 **发布上线**。一天一个文件，文件名就是日期，所以同一天再点一次新建会提示文件已存在，改成打开当天那条记录勾选即可。

想加新的坚持项：在当天记录里点「+ 习惯」加一项。**如果希望以后每天都默认带上它**，要改 `src/site.config.ts` 里的 `CHECKIN.habits`，并同步这两处：`content/_templates/checkin.md` 和 `public/admin/config.yml`（`node --test tests/checkin.test.mjs` 会检查它们是否一致，不一样会报错）。只加当天记录里的那一项不会影响别的日子。

`.`pages.yml` 里没配习惯清单。Pages CMS 的字段配置没有「列表」这个类型，可增删的条目要写成 `type: object` 加 `list`，而它的列表项**不支持默认值**，所以新建时清单不会自动列出来，得逐条点「Add an item」再打一遍习惯名。日常打卡走 `/admin/`，那边打开就已经列好了。

命令行也行：`pnpm new checkin` 生成当天的文件（按北京时间算「今天」，习惯清单照样列好）。

页面上有几个数字的含义，先说清楚：

- **连续 N 天**：从今天往回数，今天还没打勾就从昨天算起（今天没过完，不算断）。中间哪一天完全没有记录，连续就断了——所以连续的前提是这段日子你一直在记。
- **近 30 天 N/M**：M 是这 30 天里**有记录**的天数，不是 30。没记录的那天不会算成「没做到」。
- 页面上的「今天」是**上次上线构建那一天**。每天打卡都会产生一次提交、重新构建，通常就是当天；连着几天不打卡时它会停在最后一次构建的日期，新建一条记录就会对上。
